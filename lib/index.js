...
function checkoutSelectedBranch(dir, branchName) {
	...
	return exec(`cd ${dir} ${cmdSep} git checkout ${branchName} ${cmdSep} git fetch ${cmdSep} git reset --hard origin/master ${cmdSep} git pull`, spinner, (resolve, stdout) => {
			resolve(stdout);
		}, (reject, stderr) => {
...
program
	...
	.action((repository) => {
		startBuild(repository).then(() => {
			log.info('All Done O(∩_∩)O~ 大吉大利 今晚吃鸡');
